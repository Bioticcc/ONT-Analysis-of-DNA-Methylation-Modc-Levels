...
required_options <- c(
  ...
  "chromosome-coverage", "window-coverage", "bedmethyl", "gff3",
...
input_paths <- unlist(opt[c(
  ...
  "bedmethyl", "gff3", "cpg-islands", "ccre", "intergenic"
)])
...
p_coverage_scatter <- ggplot(
  ...
  geom_point(aes(size = cpg_sites_with_valid_calls, color = pct_5mc_of_valid_calls), alpha = 0.8) +
  geom_text(vjust = -0.65, size = 3, check_overlap = TRUE) +
  ...
  labs(
    ...
    subtitle = "Large deviations can reveal chromosome-scale coverage anomalies",
    x = "Mean sequencing depth", y = "Mean valid calls per covered CpG",
...
heatmap_data <- rbindlist(list(
  eligible_windows[, .(chrom, relative_position = midpoint / length,
                       modification = "5mC", percent = pct_5mc_of_valid_calls)],
  eligible_windows[, .(chrom, relative_position = midpoint / length,
                       modification = "5hmC", percent = pct_5hmc_of_valid_calls)]
))
heatmap_data[, chrom := factor(chrom, levels = rev(chromosome_order))]
p_window_heatmap <- ggplot(heatmap_data, aes(relative_position, chrom, fill = percent)) +
  geom_tile() +
  facet_wrap(~modification, ncol = 1, scales = "free") +
  scale_fill_viridis_c(option = "C", na.value = "grey90") +
  scale_x_continuous(labels = percent_format()) +
  labs(
    title = "Genome-wide methylation heatmap",
    subtitle = "Each tile is one coverage-qualified 100 kb window",
    x = "Relative position along chromosome", y = NULL,
    fill = "% valid calls"
  ) +
...
profile_rows <- rbindlist(lapply(seq_len(nrow(profile_windows)), function(index) {
  ...
  cpg[chrom == as.character(region_row$chrom) & start >= region_row$start & end <= region_row$end &
      ...
      .(category = region_row$category, region = region_row$region,
...
if (nrow(profile_rows) > 0L) {
  profile_long <- melt(profile_rows,
    id.vars = c("category", "region", "position_kb", "valid_calls"),
    measure.vars = c("pct_5mc", "pct_5hmc"),
    variable.name = "modification", value.name = "percent")
  ...
  ), by = .(category, region, modification, position_bin_kb)]
...
p_feature_5hmc <- ggplot(feature_plot, aes(feature_type, weighted_5hmc_percent, fill = feature_type)) +
  ...
  labs(
    ...
    subtitle = "SixBase-compatible feature-distribution view",
...
focused_features <- feature_methylation[
  feature_type %chin% c("CpG island", "5' UTR") & measured_cpg_sites >= min_feature_cpgs
]
focused_features[, feature_type := factor(feature_type, levels = c("CpG island", "5' UTR"))]
p_focused_5mc <- ggplot(focused_features, aes(feature_type, weighted_5mc_percent, fill = feature_type)) +
  geom_boxplot(outlier.shape = NA) +
  scale_fill_manual(values = c("CpG island" = "#4C78A8", "5' UTR" = "#E45756"), guide = "none") +
  labs(title = "CpG islands versus 5' UTRs: 5mC", x = NULL,
       y = "Region-level 5mC (% of valid calls)")
save_plot(p_focused_5mc, "19_CpG_Island_vs_5UTR_5mC", 8, 6)

p_focused_5hmc <- ggplot(focused_features, aes(feature_type, weighted_5hmc_percent, fill = feature_type)) +
  geom_boxplot(outlier.shape = NA) +
  scale_fill_manual(values = c("CpG island" = "#4C78A8", "5' UTR" = "#E45756"), guide = "none") +
  labs(title = "CpG islands versus 5' UTRs: 5hmC", x = NULL,
       y = "Region-level 5hmC (% of valid calls)")
save_plot(p_focused_5hmc, "20_CpG_Island_vs_5UTR_5hmC", 8, 6)
...
save_plot(p_feature_summary, "21_Feature_Weighted_Methylation_Summary", 12, 6.5)
...
sixbase_mapping <- data.table(
  sixbase_figure_family = c(
    ...
    "Feature methylation boxplots", "CpG island versus 5' UTR boxplots",
  ...
  stage05_output = c(
    ...
    "14-16_Feature_Methylation_Boxplots", "19-20_CpG_Island_vs_5UTR",
    "13_Extreme_Window_Regional_Profiles", "09-10_Genomewide_100kb_Windows",
    rep("NA", 6L)
  ),
  rationale = c(
    rep("The same quantity is available from one ONT sample.", 8L),
...
report_lines <- c(
  ...
  "",
  "Output guide",
  "  figures/: PDF and PNG versions of every visualization.",
